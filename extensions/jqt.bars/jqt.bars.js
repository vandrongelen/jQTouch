              ...
              for (i in iscroll.options) {
                if (iscroll.options.hasOwnProperty(i)) {
                  if (typeof($this.attr(i)) !== 'undefined') {
                    switch ($this.attr(i).toLowerCase()) {
                        case 'true':
                            iscroll.options[i] = true;
                            break;
                        case 'false':
                            iscroll.options[i] = false;
                            break;
                        default:
                            iscroll.options[i] = $this.attr(i);
                    }
                    // iscroll.options[i] = $this.attr(i).toLowerCase() === 'true' ? true : false; // this was the original setting
                  }
                }                
              }

              // To make pulling work we have to give the proper options at initialization time
              // HACK HACK HACK HACK
              iscroll.destroy();
              // iscroll.options['desktopCompatibility'] = true;
              iscroll = new iScroll(scrollID, iscroll.options);
